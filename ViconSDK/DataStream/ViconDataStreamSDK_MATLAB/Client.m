...
%% GetVersion
    function [Output] = GetVersion( obj )
      ...
      vRevision = 0;
      pRevision = libpointer( 'uint32Ptr', vRevision );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetVersion_v2', obj.InstanceID, pMajor, pMinor, pPoint, pRevision );

      % Copy out data
      Major = get( pMajor, 'Value' );
      Minor = get( pMinor, 'Value' );
      Point = get( pPoint, 'Value' );
      Revision = get( pRevision, 'Value' );
      Output = struct( 'Major', Major, 'Minor', Minor, 'Point', Point, 'Revision', Revision );
    end% GetVersion
...
%% EnableLightweightSegmentData
    function [Output] = EnableLightweightSegmentData( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      vConnected = calllib( 'ViconDataStreamSDK_MATLAB', 'EnableLightweightSegmentData', obj.InstanceID, pResult );
...
%% EnableMarkerRayData
    function [Output] = EnableMarkerRayData( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      vConnected = calllib( 'ViconDataStreamSDK_MATLAB', 'EnableMarkerRayData_v2', obj.InstanceID, pResult );
...
%% EnableCentroidData
    function [Output] = EnableCentroidData( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      vConnected = calllib( 'ViconDataStreamSDK_MATLAB', 'EnableCentroidData_v2', obj.InstanceID, pResult );
...
%% DisableLightweightSegmentData
    function [Output] = DisableLightweightSegmentData( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      vConnected = calllib( 'ViconDataStreamSDK_MATLAB', 'DisableLightweightSegmentData', obj.InstanceID, pResult );
...
%% DisableMarkerRayData
    function [Output] = DisableMarkerRayData( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      vConnected = calllib( 'ViconDataStreamSDK_MATLAB', 'DisableMarkerRayData_v2', obj.InstanceID, pResult );
...
%% DisableCentroidData
    function [Output] = DisableCentroidData( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      vConnected = calllib( 'ViconDataStreamSDK_MATLAB', 'DisableCentroidData_v2', obj.InstanceID, pResult );
...
%% IsLightweightSegmentDataEnabled
    function [Output] = IsLightweightSegmentDataEnabled( obj )
      % Call function
      vEnabled = calllib( 'ViconDataStreamSDK_MATLAB', 'IsLightweightSegmentDataEnabled', obj.InstanceID );
...
%% IsMarkerRayDataEnabled
    function [Output] = IsMarkerRayDataEnabled( obj )
      % Call function
      vEnabled = calllib( 'ViconDataStreamSDK_MATLAB', 'IsMarkerRayDataEnabled_v2', obj.InstanceID );
...
%% IsCentroidDataEnabled
    function [Output] = IsCentroidDataEnabled( obj )
      % Call function
      vEnabled = calllib( 'ViconDataStreamSDK_MATLAB', 'IsCentroidDataEnabled_v2', obj.InstanceID );
...
%% SetBufferSize
    function [Output] = SetBufferSize( obj, BufferSize )
      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'SetBufferSize', obj.InstanceID, BufferSize );
    end
...
%% SetApexDeviceFeedback
    function [Output] = SetApexDeviceFeedback( obj, DeviceName, bOn )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'SetApexDeviceFeedback_v2', obj.InstanceID, DeviceName, bOn, pResult );
...
%% GetServerOrientation
    function [Output] = GetServerOrientation( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vOrientation = 0;
      pOrientation = libpointer( 'uint32Ptr', vOrientation );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetServerOrientation_v2', obj.InstanceID, pResult, pOrientation );

      % Copy out data
      cResult  = Result( get( pResult,  'Value' ) );
      cOrientation = ServerOrientation( get( pOrientation,  'Value' ) );
      
      Output = struct( 'Result', cResult, 'Orientation', cOrientation );
...
%% GetFrameRateCount
    function [Output] = GetFrameRateCount( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vFrameRateCount = 0;
      pFrameRateCount = libpointer( 'uint32Ptr', vFrameRateCount );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetFrameRateCount', obj.InstanceID, pFrameRateCount, pResult );

      % Copy out data
      cResult      = Result( get( pResult, 'Value' ) );
      cFrameRateCount = get( pFrameRateCount, 'Value' );
      
      Output = struct( 'Result', cResult, 'Count', cFrameRateCount );
      
    end

%% GetFrameRateName
    function [Output] = GetFrameRateName( obj, FrameRateIndex )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      cFrameRateName = calllib( 'ViconDataStreamSDK_MATLAB', 'GetFrameRateName', obj.InstanceID, FrameRateIndex, pResult );

      % Copy out data
      cResult = Result( get( pResult, 'Value' ) );
      
      Output = struct( 'Result', cResult, 'Name', cFrameRateName );
      
    end

%% GetFrameRateValue
    function [Output] = GetFrameRateValue( obj, FrameRateName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vValue = 0.0;
      ...
      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetFrameRateValue', obj.InstanceID, FrameRateName, pValue, pResult );
...
%% GetObjectQuality
    function [Output] = GetObjectQuality( obj, SubjectName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vQuality = 0;
      pQuality = libpointer( 'doublePtr', vQuality );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetObjectQuality_v2', obj.InstanceID, SubjectName, pQuality, pResult );

      % Copy out data
      cResult   = Result( get( pResult, 'Value' ) );
      cQuality = get( pQuality, 'Value' );
            
      Output = struct( 'Result', cResult, 'Quality', cQuality );
...
%% GetMarkerRayContributionCount
    function [Output] = GetMarkerRayContributionCount( obj, SubjectName, MarkerName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vCount = 0;
      pCount = libpointer( 'uint32Ptr', vCount );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetMarkerRayContributionCount_v2', obj.InstanceID, SubjectName, MarkerName, pCount, pResult );

      % Copy out data
      cResult      = Result( get( pResult, 'Value' ) );
      cCount = get( pCount, 'Value' );
            
      Output = struct( 'Result', cResult, 'RayContributionsCount', cCount );
      
    end

%% GetMarkerRayContribution
    function [Output] = GetMarkerRayContribution( obj, SubjectName, MarkerName, MarkerRayContributionIndex )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vCameraID = 0;
      pCameraID = libpointer( 'uint32Ptr', vCameraID );
	  vCentroidIndex = 0;
	  pCentroidIndex = libpointer( 'uint32Ptr', vCentroidIndex );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetMarkerRayContribution_v2', obj.InstanceID, SubjectName, MarkerName, MarkerRayContributionIndex, pCameraID, pCentroidIndex, pResult );

      % Copy out data
      cResult      = Result( get( pResult, 'Value' ) );
      cCameraID = get( pCameraID, 'Value' );
	  cCentroidIndex = get( pCentroidIndex, 'Value');
            
      Output = struct( 'Result', cResult, 'CameraID', cCameraID, 'CentroidIndex', cCentroidIndex );
...
%% GetUnlabeledMarkerGlobalTranslation
    function [Output] = GetUnlabeledMarkerGlobalTranslation( obj, MarkerIndex )
      ...
      vMarkerID = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      pMarkerID = libpointer( 'uint32Ptr', vMarkerID );
      vTranslation = zeros( 1, 3 );
      pTranslation = libpointer( 'doublePtrPtr', vTranslation );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetUnlabeledMarkerGlobalTranslation_v2', obj.InstanceID, MarkerIndex, pTranslation, pMarkerID, pResult );

      % Copy out data
      cResult      = Result( get( pResult, 'Value' ) );
      cMarkerID    = get( pMarkerID, 'Value' );
      cTranslation = get( pTranslation, 'Value' );
      cTranslation = cTranslation';
            
      Output = struct( 'Result', cResult, 'Translation', cTranslation, 'MarkerID', cMarkerID );
      
    end

%% GetLabeledMarkerCount
    function [Output] = GetLabeledMarkerCount( obj )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vMarkerCount = 0;
      ...
      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetLabeledMarkerCount_v2', obj.InstanceID, pMarkerCount, pResult );
...
%% GetLabeledMarkerGlobalTranslation
    function [Output] = GetLabeledMarkerGlobalTranslation( obj, MarkerIndex )
      % Local Data
      vResult = 0;
      vMarkerID = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      ...
      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetLabeledMarkerGlobalTranslation_v2', obj.InstanceID, MarkerIndex, pTranslation, pMarkerID, pResult );
...
%% GetDeviceOutputName
  ...
  %% GetDeviceOutputComponentName
    function [Output] = GetDeviceOutputComponentName( obj, DeviceName, DeviceOutputIndex )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vUnit = 0;
      ...
      % Call function
      % Unit and Result should be the same for each of these calls.
      cDeviceOutputName = calllib( 'ViconDataStreamSDK_MATLAB', 'GetDeviceComponentOutputName_v2', obj.InstanceID, DeviceName, DeviceOutputIndex, pUnit, pResult );
      cDeviceOutputComponentName = calllib( 'ViconDataStreamSDK_MATLAB', 'GetDeviceComponentName_v2', obj.InstanceID, DeviceName, DeviceOutputIndex, pUnit, pResult );

      % Copy out data
      cResult = Result( get( pResult, 'Value' ) );
      cUnit   = Unit(   get( pUnit,   'Value' ) );
     
      Output = struct( 'Result', cResult, 'DeviceOutputName', cDeviceOutputName, 'DeviceOutputComponentName', cDeviceOutputComponentName, 'DeviceOutputUnit', cUnit );
...
%% GetDeviceOutputComponentSubsamples
    function [Output] = GetDeviceOutputComponentSubsamples( obj, DeviceName, DeviceOutputName, DeviceOutputComponentName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vDeviceOutputSubsamples = 0;
      ...
      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetDeviceOutputComponentSubsamples_v2', obj.InstanceID, DeviceName, DeviceOutputName, DeviceOutputComponentName, pDeviceOutputSubsamples, pOccluded, pResult );
...
%% GetDeviceOutputValue
  ...
  %% GetDeviceOutputComponentValue
    function [Output] = GetDeviceOutputComponentValue( obj, DeviceName, DeviceOutputName, DeviceOutputComponentName, Subsample )
      % Default subsample to 1 if not supplied.
      if ( nargin == 4 )
        Subsample = 1;
      end
      ...
      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetDeviceOutputComponentSubsample_v2', obj.InstanceID, DeviceName, DeviceOutputName, DeviceOutputComponentName, Subsample, pValue, pOccluded, pResult );
...
%% GetCameraCount
    function [Output] = GetCameraCount( obj )
      % Local Data
      vResult      = 0;
      pResult      = libpointer( 'uint32Ptr', vResult );
      vCameraCount = 0;
      pCameraCount = libpointer( 'uint32Ptr', vCameraCount );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetCameraCount_v2', obj.InstanceID, pCameraCount, pResult );

      % Copy out data
      cResult          = Result( get( pResult, 'Value' ) );      
      cCameraCount = get( pCameraCount, 'Value' );
      
      Output = struct( 'Result', cResult, 'CameraCount', cCameraCount );

    end
    
%% GetCameraName
    function [Output] = GetCameraName( obj, CameraIndex )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      cCameraName = calllib( 'ViconDataStreamSDK_MATLAB', 'GetCameraName_v2', obj.InstanceID, CameraIndex, pResult );

      % Copy out data
      cResult     = Result(     get( pResult,     'Value' ) );
      
      Output = struct( 'Result', cResult, 'CameraName', cCameraName );
      
    end

%% GetCameraId
    function [Output] = GetCameraId( obj, CameraName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vId = 0;
      pId = libpointer( 'uint32Ptr', vId );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetCameraId_v2', obj.InstanceID, CameraName, pId, pResult );

      % Copy out data
      cResult     = Result(     get( pResult,     'Value' ) );
      cId         = get( pId,      'Value');

      Output = struct( 'Result', cResult, 'CameraId', cId );
      
    end

%% GetCameraUserId
    function [Output] = GetCameraUserId( obj, CameraName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vUserId = 0;
      pUserId = libpointer( 'uint32Ptr', vUserId );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetCameraId_v2', obj.InstanceID, CameraName, pUserId, pResult );

      % Copy out data
      cResult     = Result(     get( pResult,     'Value' ) );
      cUserId     = get( pUserId,      'Value');

      Output = struct( 'Result', cResult, 'CameraUserId', cUserId );
      
    end

%% GetCameraType
    function [Output] = GetCameraType( obj, CameraName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      cCameraType = calllib( 'ViconDataStreamSDK_MATLAB', 'GetCameraType_v2', obj.InstanceID, CameraName, pResult );

      % Copy out data
      cResult     = Result(     get( pResult,     'Value' ) );
      
      Output = struct( 'Result', cResult, 'CameraType', cCameraType );
      
    end

%% GetCameraDisplayName
    function [Output] = GetCameraDisplayName( obj, CameraName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );

      % Call function
      cCameraDisplayName = calllib( 'ViconDataStreamSDK_MATLAB', 'GetCameraDisplayName_v2', obj.InstanceID, CameraName, pResult );

      % Copy out data
      cResult     = Result(     get( pResult,     'Value' ) );
      
      Output = struct( 'Result', cResult, 'CameraDisplayName', cCameraDisplayName );
      
    end

%% GetCameraResolution
    function [Output] = GetCameraResolution( obj, CameraName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vResolutionX = 0;
      pResolutionX = libpointer( 'uint32Ptr', vResolutionX );
      vResolutionY = 0;
      pResolutionY = libpointer( 'uint32Ptr', vResolutionY );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetCameraResolution_v2', obj.InstanceID, CameraName, pResolutionX, pResolutionY, pResult );

      % Copy out data
      cResult     = Result(     get( pResult,           'Value' ) );
     cResolutionX = get( pResolutionX,      'Value');
     cResolutionY = get( pResolutionY,      'Value');

      Output = struct( 'Result', cResult, 'ResolutionX', cResolutionX, 'ResolutionY', cResolutionY );
      
    end

%% GetIsVideoCamera
    function [Output] = GetIsVideoCamera( obj, CameraName )
      % Local Data
      vResult = 0;
      pResult = libpointer( 'uint32Ptr', vResult );
      vIsVideoCamera = 0;
      pIsVideoCamera = libpointer( 'uint32Ptr', vIsVideoCamera );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetIsVideoCamera_v2', obj.InstanceID, CameraName, pIsVideoCamera, pResult );

      % Copy out data
      cResult            = Result(     get( pResult,        'Value' ) );
      cIsVideoCamera     = get( pIsVideoCamera, 'Value');

      Output = struct( 'Result', cResult, 'IsVideoCamera', cIsVideoCamera );
      
    end

%% GetCentroidCount
    function [Output] = GetCentroidCount( obj, CameraName )
      % Local Data
      vResult      = 0;
      pResult      = libpointer( 'uint32Ptr', vResult );
      vCentroidCount = 0;
      pCentroidCount = libpointer( 'uint32Ptr', vCentroidCount );

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetCentroidCount_v2', obj.InstanceID, CameraName, pCentroidCount, pResult );

      % Copy out data
      cResult          = Result( get( pResult, 'Value' ) );      
      cCentroidCount = get( pCentroidCount, 'Value' );
      
      Output = struct( 'Result', cResult, 'CentroidCount', cCentroidCount );

    end

%% GetCentroidPosition
    function [Output] = GetCentroidPosition( obj, CameraName, CentroidIndex )
      % Local Data
      vResult     = 0;
      pResult     = libpointer( 'uint32Ptr', vResult );
      vPosition = zeros( 1, 2 );
      pPosition = libpointer( 'doublePtrPtr', vPosition );
      vRadius   = 0;
      pRadius   = libpointer( 'doublePtr', vRadius ); 

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetCentroidPosition_v2', obj.InstanceID, CameraName, CentroidIndex, pPosition, pRadius, pResult );

      % Copy out data
      cResult     = Result( get( pResult, 'Value' ) );
      cPosition = get( pPosition, 'Value' );
      cPosition = cPosition';
      cRadius   = get( pRadius, 'Value' );
            
      Output = struct( 'Result', cResult, 'Position', cPosition, 'Radius', cRadius );
    end
      
%% GetCentroidWeight
    function [Output] = GetCentroidWeight( obj, CameraName, CentroidIndex )
      % Local Data
      vResult     = 0;
      pResult     = libpointer( 'uint32Ptr', vResult );
      vWeight     = 0;
      pWeight     = libpointer( 'doublePtr', vWeight ); 

      % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'GetCentroidWeight_v2', obj.InstanceID, CameraName, CentroidIndex, pWeight, pResult );

      % Copy out data
      cResult     = Result( get( pResult, 'Value' ) );
      cWeight   = get( pWeight, 'Value' );
            
      Output = struct( 'Result', cResult, 'Weight', cWeight );
    end

%% ClearSubjectFilter
    function [Output] = ClearSubjectFilter( )
    % LocalData
      vResult     = 0;
      pResult     = libpointer( 'uint32Ptr', vResult );

    % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'ClearSubjectfilter', obj.InstanceID, pResult );
...
%% AddToSubjectFilter
    function [Output] = AddToSubjectFilter( obj, SubjectName )
    % LocalData
      vResult     = 0;
      pResult     = libpointer( 'uint32Ptr', vResult );

    % Call function
      calllib( 'ViconDataStreamSDK_MATLAB', 'AddToSubjectFilter', obj.InstanceID, SubjectName, pResult );
